...
class Fix1003Rule(BaseRule):
    ...
    def run_rule(self, project: StructuredProjectSource,
        ...
        k = self.find_true_originline_number(self.all_lines,warning_last_line)
        one_line_warning_code = True

        start = self.find_true_originline_number(self.all_lines,warning_first_line)
        # self.remove_code(warning_first_line,warning_last_line)
        # self.replace_useless_multiple_line(start-1,k,all_lines)

        for i in range(k+1, len(self.all_lines)):
            ...
            if ";" not in self.all_lines[i].code:
                ...
                #i_position = self.replace_useless_multiple_line(start-1, i, self.all_lines)
                i_position = self.delete_dpct_warning(warning_begin_id, warning_end_id)
                new_lineItem = LineItem(new_code)
                self.all_lines.insert(i_position,new_lineItem)
                self.test_print( self.all_lines, warning_last_line)
