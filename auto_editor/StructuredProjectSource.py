...
class StructuredProjectSource:
    ...
    def get_all_dpct_file_paths(self) -> List[str]:
        ...
        '''
        # Todo: use same logic here as in PreAnalyser
    ...
    def get_dpct_warnings_dict(self) -> Dict[str: List[WarningLocation]]:
        ...
        for file_path, code_lines in self.paths_to_lines.items():
            for i in range(len(code_lines)):
                ...
                if warning_code:
                    first_line = self.get_first_warning_line(i, code_lines).id
                    last_line = self.get_last_warning_line(i, code_lines).id

                    if not warnings_dict[warning_code]:
