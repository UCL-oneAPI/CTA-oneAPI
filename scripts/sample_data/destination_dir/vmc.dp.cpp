...
/*
CTA1032:0: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
*/
template <typename T>
__dpct_inline__ T rand(oneapi::mkl::rng::device::philox4x32x10<1> *state);

/*
CTA1032:1: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
*/
template <>
__dpct_inline__ float
rand<float>(oneapi::mkl::rng::device::philox4x32x10<1> *state) {
  oneapi::mkl::rng::device::uniform<float> distr_ct1;
  return oneapi::mkl::rng::device::generate(distr_ct1, *state);
}

/*
CTA1032:2: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
*/
template <>
__dpct_inline__ double
rand<double>(oneapi::mkl::rng::device::philox4x32x10<1> *state) {
  /*
...
void SumWithinBlocks(const int n, const FLOAT* data, FLOAT* blocksums,
  ...
  /*
  CTA1065:4: CTA recommended to ignore this warning. 
  ...
  */
  ...
  for (unsigned int s=pow2>>1; s>0; s>>=1) {
    ...
    /*
    CTA1065:5: CTA recommended to ignore this warning. 
    but you can also consider replacing 'item_ct1.barrier();' 
    with 'item_ct1.barrier(sycl::access::fence_space::local_space);' 
    to have have better performance if the kernel function 
    has no memory accesses in the global memory.
...
/*
CTA1032:6: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
*/
...
// initializes samples
/*
CTA1032:7: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
*/
...
void propagate(
    ...
    /*
    CTA1032:8: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
    */
...
int main() {
  ...
  /*
  CTA1032:9: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
  */
  oneapi::mkl::rng::device::philox4x32x10<1> *ranstates;

  x1 = sycl::malloc_device<FLOAT>(Npoint, q_ct1);
  y1 = sycl::malloc_device<FLOAT>(Npoint, q_ct1);
  z1 = sycl::malloc_device<FLOAT>(Npoint, q_ct1);
  x2 = sycl::malloc_device<FLOAT>(Npoint, q_ct1);
  y2 = sycl::malloc_device<FLOAT>(Npoint, q_ct1);
  z2 = sycl::malloc_device<FLOAT>(Npoint, q_ct1);
  psi = sycl::malloc_device<FLOAT>(Npoint, q_ct1);
  stats = sycl::malloc_device<FLOAT>(4 * Npoint, q_ct1);
  blocksums = sycl::malloc_device<FLOAT>(NBLOCK, q_ct1); // workspace for summation
  statsum = sycl::malloc_device<FLOAT>(4, q_ct1); // workspace for summation
  ranstates =sycl::malloc_device<oneapi::mkl::rng::device::philox4x32x10<1>>(Npoint, q_ct1);
  /*
  CTA1032:21: Base on the experience, recommended to ignore this warning. If it didn't work, adjust the code.
  */
  ...
  for (int sample=0; sample<Nsample; sample++) {
    ...
    q_ct1.memcpy(&s, statsum, sizeof(s)).wait();
  ...
  sycl::free(blocksums, q_ct1);
  sycl::free(statsum, q_ct1);
  sycl::free(ranstates, q_ct1);
  return 0;
}
