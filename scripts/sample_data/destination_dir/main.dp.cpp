                         ...
                         const float othreshold, sycl::nd_item<3> item_ct1)
{
  ...
  if (rects[i].w() < rects[j].w())
  {
    ...
    float w = sycl::fmax(
        0.0f,
        sycl::fmin(rects[i].x() + rects[i].z(), rects[j].x() + rects[j].z()) -
            sycl::fmax((float)(rects[i].x()), (float)(rects[j].x())) + 1.0f);
    float h = sycl::fmax(
        0.0f,
        sycl::fmin(rects[i].y() + rects[i].z(), rects[j].y() + rects[j].z()) -
            sycl::fmax((float)(rects[i].y()), (float)(rects[j].y())) + 1.0f);
...
{
  /*
  CTA1065:0: CTA recommended to ignore this warning. 
...
int main(int argc, char *argv[]) try {
  ...
  sycl::float4 *points;
  points = sycl::malloc_device<sycl::float4>(MAX_DETECTIONS, q_ct1);

  q_ct1.memset(points, 0, sizeof(sycl::float4) * MAX_DETECTIONS).wait();

  /* GPU array for storing the non-maximum supression bitmap */
  unsigned char* nmsbitmap;
  nmsbitmap =(unsigned char *)sycl::malloc_device(sizeof(unsigned char) * MAX_DETECTIONS * MAX_DETECTIONS, q_ct1);
  ...
  q_ct1.memset(nmsbitmap, 1,sizeof(unsigned char) * MAX_DETECTIONS * MAX_DETECTIONS).wait();

  q_ct1.memset(pointsbitmap, 0, sizeof(unsigned char) * MAX_DETECTIONS).wait();

  /* Transfer detection coordinates read from the input text file to the GPU */
  q_ct1.memcpy(points, cpu_points, sizeof(sycl::float4) * MAX_DETECTIONS).wait();
  ...
  */
  ...
  q_ct1.memcpy(cpu_pointsbitmap, pointsbitmap,sizeof(unsigned char) * MAX_DETECTIONS).wait();
